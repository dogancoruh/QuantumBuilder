...
namespace Quantum.GenericProperty.Utilities
{
    ...
    {
        public static string FixStringNumberToDigitCount(string numberText, int digitCount)
        {
            var result = numberText;

            if (digitCount - numberText.Length > 0)
            {
                for (int i = 0; i < digitCount - numberText.Length; i++)
                    result += "0";
            }
